...
def label2color(n_classes, seg):
    VOC_COLORMAP = [[255, 0, 0], [255, 255, 0], [255, 255, 255], [0, 255, 0],
                    [0, 255, 255], [0, 0, 255]]
    # VOC_COLORMAP = [[0, 0, 0], [128, 128, 0], [0, 0, 128], [128, 0, 0],
    #                 [0, 128, 0]]
    seg_color = np.zeros((seg.shape[0], seg.shape[1], 3))
    for c in range(n_classes):
        seg_color[:, :, 0] += ((seg == c) *
                               (VOC_COLORMAP[c][0])).astype('uint8')
        seg_color[:, :, 1] += ((seg == c) *
                               (VOC_COLORMAP[c][1])).astype('uint8')
        seg_color[:, :, 2] += ((seg == c) *
                               (VOC_COLORMAP[c][2])).astype('uint8')
